...
@Component({
  selector: "app-subscribe-paid-groups",
  templateUrl: "./subscribe-paid-groups.component.html",
  styleUrls: ["./subscribe-paid-groups.component.scss"],
  providers: [{
...
export class SubscribePaidGroupsComponent implements OnInit {
  ...
  ngDoCheck() {
    if ((!this.activeWorkspace && !this.activeWorkspace._id) || (this.activeWorkspace._id !== this.homeService.activeWorkspace._id)) {
      this.loadPaidWorkspace();
    }
  }

  loadPaidWorkspace() {
    this.activeWorkspace = this.homeService.activeWorkspace;
    if (!this.homeService.activeWorkspace.name) {
      this.activeWorkspace = this.helperService.getLocalStore('activeWorkspace');
      this.homeService.activeWorkspace = this.activeWorkspace;
    }
    if (this.activeWorkspace.paymentOptions) {
      this.selectedPlan = this.activeWorkspace.paymentOptions[0];
    }
  }

  loadYTVideo(){
    if(this.activeWorkspace && this.activeWorkspace.upsell && this.activeWorkspace.upsell.videoLink){
      let reLink = this.activeWorkspace.upsell.videoLink.split('/');
      let reLink2 = reLink[reLink.length - 1].split('=');
      let ytVideoIframe = `<iframe width="560" height="315" src="https://www.youtube.com/embed/${reLink2[reLink2.length - 1]}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>`;
  ...
  makePayment(){
    ...
    if(!this.cardNumber || !this.expMonth || !this.expYear || !this.cvcNumber || !this.country || !this.zip){
    ...
    const data = {
        ...
        organizationId: this.activeWorkspace.organization_id,
